class KB(object):
	def __init__(self):
		self.entities = {}# {entity1:[relation entity2]}
	...
	def addRelation(self, entity1, relation, entity2):
		...
		# if entity1 in self.entities.keys():
		# 	self.entities[entity1].append("\t{}\t{}".format(relation, entity2))
		# else:
		# 	self.entities[entity1] = ["\t{}\t{}".format(relation, entity2)]
	...
	def pickRandomIntermediatesBetween(self, entity1, entity2, num):
		...
		for i in range(num):
			# s = self.entities.keys()
			itermediate = random.choice(list(self.entities.keys()))
			while itermediate in res or itermediate == entity1 or itermediate == entity2: # 若随机结果已经在res中，或是entity1, entity2的话，则重新随机选择
				itermediate = random.choice(list(self.entities.keys()))
			res.add(itermediate)
		# a = list(res)
		return list(res)
